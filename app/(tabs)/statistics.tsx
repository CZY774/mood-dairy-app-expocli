...
import { Appbar, Text, useTheme, SegmentedButtons, Card } from 'react-native-paper';
...
export default function StatisticsScreen() {
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <Appbar.Header>
        <Appbar.Content title="📊 Statistik Mood" titleStyle={styles.headerTitle} />
      </Appbar.Header>
        ...
        <View style={styles.content}>
          ...
          {stats && (
            <StatisticsChart
              stats={stats}
              title={`Grafik Mood ${
                selectedPeriod === 'week' ? '7 Hari Terakhir' :
                selectedPeriod === 'month' ? '30 Hari Terakhir' : '90 Hari Terakhir'
              }`}
            />
          )}
                ...
                {topActivities.map((item, index) => (
                  <View key={item!.activity.id} style={styles.activityItem}>
                    <View style={styles.activityInfo}>
                      <Text style={styles.activityEmoji}>{item!.activity.emoji}</Text>
                      <Text variant="bodyMedium" style={[styles.activityName, { color: theme.colors.onSurface }]}>
                        {item!.activity.name}
                      </Text>
                    </View>
                    <View style={styles.activityCount}>
                      <Text variant="bodyMedium" style={[styles.countText, { color: theme.colors.primary }]}>
                        {item!.count}x
                      </Text>
                    </View>
                  </View>
              ...
              <Card.Content>
                ...
                <View style={styles.insightItem}>
                  <Text variant="bodyMedium" style={[styles.insightText, { color: theme.colors.onSurface }]}>
                    ...
                    {stats.averageMood >= 4 ? ' - Kamu terlihat bahagia! 😊' :
                     stats.averageMood >= 3 ? ' - Mood kamu cukup stabil 😌' :
            ...
            <Card style={[styles.emptyCard, { backgroundColor: theme.colors.surface }]}>
              <Card.Content>
                ...
                <Text variant="bodyMedium" style={[styles.emptySubText, { color: theme.colors.onSurfaceVariant }]}>
                  Mulai catat mood harianmu untuk melihat statistik dan insight menarik!
                </Text>
              </Card.Content>
            </Card>
          )}
